...
for i=1:length(file_name) %from 1 to N (N = number of names (or rows) in array file_name)
   ...
   %corresponds to which condition and which bin
    if name_temp(3) == 'B' 
        events =[currentDirectory '/EVENTLIST-Block.txt']; %the backslash before EVENTLIST just allows MATLAB to create a path and find the txt file. Current directory points to the path where the txt file needs to be looked for.
    elseif name_temp(2) == 'M'
...
%j is defined here by the line after 'for'
  for j=1:2  
   ...
   name_temp = name_temp(1:end-4);  %name_temp = name_temp([1:length(name_temp)-4]);
    ...
    if j==1 % human 1 
      nameerp = [name_temp(1:2) 'H1' '.erp'];
      electrodes=[7 8 10:16 19:26 28:34];%defines which electrodes are included for H1 apart from frontal elec
      frontals = [3:6]; %defines frontal electrodes (processing parameters are more strict for frontals)
      placingelectrode = {'nch1 = ch3 label Fp2',  'nch2 = ch4 label Fp1',  'nch3 = ch5 label F8',  'nch4 = ch6 label F7',...
                        ...
                        'nch27 = ch33 label O2',  'nch28 = ch34 label O1'}; %exclude unused channels (ch1, ch2, ch9 etc) and create the liste of 28 channels
     elseif j==2 %human 2 %same but for the other participant and second half of channel matrix
        nameerp = [name_temp(1:2) 'H2' '.erp'];
        electrodes= [43 44 46:52 55:62 64:70]; 
        frontals= [39:42]; 
